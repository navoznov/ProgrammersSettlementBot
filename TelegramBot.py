...
class TelegramBot:
    ...
    def schedule_message(self, chat_id, time):
    ...
    def broadcastTemperature(self, temperature):
        ...
        for update in updates:
            ...
            if text == '/start':
                ...
                start_message = self.start_message.format('@' + username)
                self.send_message(chat_id, start_message)
            elif text == '/schedule' or text == '/plan':
                self.send_message(chat_id,  self.schedule_question_message)
                self.schedule_awaiting_chat_ids.add(chat_id)
            elif chat_id in self.schedule_awaiting_chat_ids:
                is_success, time = self.parseTime(text)
                if is_success:
                    if self.schedule_message(chat_id, time):
                        hours, minutes = time
                        message = self.schedule_success_message.format(hours.zfill(2), minutes.zfill(2))
                        self.send_message(chat_id, )
                        self.schedule_awaiting_chat_ids.remove(chat_id)
                    else:
                        self.send_message(chat_id, self.schedule_fail_message)
                else:
                    self.send_message(chat_id, self.schedule_parsing_fail_message)

            elif not chat_id in notified_chat_ids:
                self.send_message(chat_id, temperature_message)
